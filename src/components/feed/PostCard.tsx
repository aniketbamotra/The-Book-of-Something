  ...
  return parts.map((part, i) => {
    if (part.startsWith("```")) {
      ...
      return (
        ...
        >
          {lang && (
            <div className="text-indigo-400/60 text-xs mb-2 font-sans tracking-wider uppercase">
              {lang}
            </div>
          )}
          <code>{code}</code>
    ...
    const rendered = part.split("\n").map((line, j) => {
      ...
      const withCode = formatted.replace(
        /`([^`]+)`/g,
        '<code class="bg-indigo-500/15 text-indigo-300 px-1.5 py-0.5 rounded text-sm font-mono border border-indigo-500/20">$1</code>'
      );

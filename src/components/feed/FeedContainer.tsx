...
const CompletionScreen = dynamic(
  () =>
    import("@/components/feed/CompletionScreen").then(
      (m) => m.CompletionScreen
    ),
  { ssr: false }
);
...
export function FeedContainer({ data }: FeedContainerProps) {
  ...
  useEffect(() => {
    ...
    const targetIndex = posts.findIndex(
      (p) => p.order === progress.currentPostOrder
    );
    if (targetIndex > 0) {
  ...
  const avgScore =
    progress && progress.quizHistory.length > 0
      ? progress.quizHistory.reduce((sum, q) => sum + q.score, 0) /
        ...
        {showQuiz && (
          <QuizOverlay
            ...
            score={
              ...
              Math.max(1, quiz.answers.length)
            }
            difficultyChanged={
              quiz.state === "complete" &&

import type { UserProgress, CourseProgress, QuizAttempt, DifficultyLevel } from '@/types'
...
export function updatePostProgress(courseId: string, postId: string, postOrder: number): void {
  const progress = loadProgress()
...
export function recordQuizAttempt(courseId: string, attempt: QuizAttempt): void {
  const progress = loadProgress()
  const cp = progress.courses[courseId]
  if (!cp) return
...
export function resetCourseProgress(courseId: string, difficulty: DifficultyLevel): void {
  const progress = loadProgress()
  const now = new Date().toISOString()
